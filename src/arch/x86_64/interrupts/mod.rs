...
static mut INTERRUPTS: Mutex<InterruptDescriptorTable> = Mutex::new(InterruptDescriptorTable::new());


extern "x86-interrupt" fn exception_handler(stack_frame: &mut ExceptionStackFrame, error_code: u64)
{
  println!("EXCEPTION:");
  println!("{:#?}", stack_frame);
...
type PageFaultHandler = extern "x86-interrupt" fn(_: &mut ExceptionStackFrame, _: PageFaultErrorCode);
...
pub fn set_general_protection_fault_handler(handler: ExceptionHandler) {
  unsafe {
    INTERRUPTS.lock().general_protection_fault.set_handler_fn(handler);
  }
}
...
pub fn install() {
  ...
  extern "x86-interrupt" fn gpf_handler(stack: &mut ExceptionStackFrame, error_code: u64) {
    panic!("General protection fault!\nError code {}, stack: {:?}", error_code, stack);
  };

  extern "x86-interrupt" fn double_fault_handler(stack: &mut ExceptionStackFrame, error_code: u64) {
    panic!("Double fault!\nError code {}, stack: {:?}", error_code, stack);
  };



  interrupts.general_protection_fault.set_handler_fn(gpf_handler);
  ...
  unsafe {
    ...
    println!("Installing IDT: base={:016x?}, limit={:08x?}", idt_base, idt_limit);
